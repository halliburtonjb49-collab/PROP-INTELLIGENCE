...
class _PropBuilderScreenState extends State<PropBuilderScreen> {
  ...
  String _buildSlipExportText() {
    ...
    if (legs.isEmpty) {
      return 'The Daily Spin\nNo picks selected.';
    }
    final buffer = StringBuffer();
    buffer.writeln('THE DAILY SPIN');
    ...
    buffer.writeln();
    buffer.writeln('Generated by The Daily Spin');
  ...
  Future<void> _saveSlipAsImage() async {
    ...
    try {
      ...
      final path = await FilePicker.saveFile(
        dialogTitle: 'Save Daily Spin Slip',
        fileName: 'daily_spin_slip.png',
  ...
  Future<void> _saveSlipAsPdf() async {
    ...
    try {
      ...
      final path = await FilePicker.saveFile(
        dialogTitle: 'Save Daily Spin PDF',
        fileName: 'daily_spin_slip.pdf',
        type: FileType.custom,
